...
app.post("/add-watermark", async (req: Request, res: Response) => {
    try {
        ...
        const font = await Jimp.loadFont(req.body.font.color === FontColor.BLACK ? fontPicker.selectBlackFontSize(req.body.font.size) : fontPicker.selectWhiteFontSize(req.body.font.size));
        ...
        //@ts-ignore
        image.getBase64(Jimp.AUTO, (err: any, base64: any) => {
            if (err) {
